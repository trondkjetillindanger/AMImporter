...
namespace AMImporter
{
    public class TimeSchedule
    {
        ...
        Dictionary<string, AMEvent> AMEvents = null;
        public TimeSchedule(string _path)
        {
            string fullFileName = _path + "\\" + filename;
            AMEvents = File.ReadLines(fullFileName).Select(line => line.Split(';')).ToDictionary(line => line[1], line => new AMEvent { Name = line[1], Time = line[0], SAEventCategoryName = line[2], AgeCategory = line[3].Split(',').ToList() });
        }
